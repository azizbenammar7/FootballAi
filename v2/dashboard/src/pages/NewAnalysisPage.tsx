    ...
    <div className="page">
      ...
      <form className="analysis-form" onSubmit={submit}>
        ...
        <Panel>
          ...
          {profile && <div className={`profile-card ${profile.available ? '' : 'profile-card--unavailable'}`}><strong>{profile.display_name}</strong><p>{profile.description}</p><small>{profile.available ? profile.purpose : `Missing: ${profile.missing_requirements.join(', ')}`}</small>{profile.warnings.map((warning) => <span key={warning}>{warning}</span>)}</div>}
          {profiles.data?.profiles.filter((item) => !item.available).map((item) => <p className="muted" key={item.profile_id}>{item.display_name} unavailable — missing: {item.missing_requirements.join(', ')}</p>)}
