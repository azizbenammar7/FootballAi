...
def main() -> None:
    ...
    if any(run.status.value == "succeeded" and run.parameters.get("pipeline_profile") == "demo_fast" for run in coordinator.store.list_runs()):
        return
    with tempfile.TemporaryDirectory(prefix="footballai-demo-") as temporary:
        ...
        status = AnalysisExecutor(coordinator.store, stage_delay_seconds=0).execute(job, "demo-seed")
